...
class ResearchDBValidator:
    ...
    def validate_all(self) -> bool:
        ...
        md_files = [
            f for f in md_files if not any(part.startswith("_") for part in f.parts)
        ]
    ...
    def _validate_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            self.errors.append(
                ValidationError(
                    str(file_path.relative_to(self.db_path)), 1, f"Invalid YAML: {e}"
                )
            )
            return
    ...
    def _validate_metadata(self, file_path: Path, metadata: dict) -> None:
        ...
        for field in required_fields:
            if field not in metadata:
                self.errors.append(
                    ValidationError(rel_path, 0, f"Missing required field: '{field}'")
                )
        ...
        if "tags" in metadata:
            if not isinstance(metadata["tags"], list):
                self.errors.append(ValidationError(rel_path, 0, "Tags must be a list"))
            elif len(metadata["tags"]) == 0:
                self.warnings.append(
                    ValidationError(rel_path, 0, "No tags specified", "warning")
                )
        ...
        if not re.match(r"adr-\d+-", metadata.get("id", "")):
            self.warnings.append(
                ValidationError(
                    rel_path,
                    0,
                    "Decision ID should follow pattern: adr-XXX-title",
                    "warning",
                )
            )
    ...
    def _validate_body(self, file_path: Path, doc_type: str, body: str) -> None:
        ...
        if "## Relations" not in body:
            self.errors.append(
                ValidationError(rel_path, 0, "Missing '## Relations' section")
            )
                ...
                if link not in self.all_ids:
                    self.warnings.append(
                        ValidationError(
                            rel_path,
                            0,
                            f"Broken link: '[[{link}]]' does not exist",
                            "warning",
                        )
                    )
    ...
    def _report_results(self) -> bool:
        ...
        if not self.errors and not self.warnings:
            ...
            print(
                f"{Colors.GREEN}✓ No errors (warnings can be addressed later){Colors.END}\n"
            )
            return True
        else:
            print(
                f"{Colors.RED}✗ Validation failed. Please fix errors before merging.{Colors.END}\n"
            )
            return False
