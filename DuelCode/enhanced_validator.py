...
class DuelCodeEnhancedValidator:
    ...
    SUPPORTED_LANGUAGES = {
        ...
        'go', 'rust', 'ruby', 'php', 'swift', 'kotlin', 'dart', 'html', 'css',
        'sql', 'json', 'yaml', 'markdown', 'mermaid'
    }

    # Common programming language file extensions
    CODE_EXTENSIONS = {
        '.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.c', '.cpp', '.h',
        '.hpp', '.cs', '.go', '.rs', '.rb', '.php', '.swift', '.kt', '.dart',
    ...
    def validate_has_checklist(self) -> None:
        ...
        if not checklist:
            self._add_result(
                "Missing 'Before You Start' checklist",
                Severity.ERROR,
                0
            )
            return
        ...
        for i, line in enumerate(self.lines):
            if line.startswith('```'):
                if in_code_block and code_block_lines:
                    self._analyze_code_block(code_block_lines, current_lang, i - len(code_block_lines) - 1)
                    code_block_lines = []
                else:
    ...
    def _analyze_code_block(self, lines: List[Tuple[int, str]], lang: str, start_line: int) -> None:
    ...
    def _analyze_python_code(self, lines: List[Tuple[int, str]], start_line: int) -> None:
    ...
    def _analyze_javascript_code(self, lines: List[Tuple[int, str]], start_line: int) -> None:
    ...
    def validate_has_glossary(self) -> None:
        ...
        technical_terms = [
            'API', 'REST', 'GraphQL', 'OAuth', 'JWT', 'Docker', 'Kubernetes',
            'Microservices', 'CI/CD', 'TDD', 'BDD', 'MVC', 'ORM', 'SQL', 'NoSQL'
        ]
