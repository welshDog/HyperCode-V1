...
class Environment:
    ...
    def get(self, name: str):
        # We access the singleton here to avoid passing it through every env
        TelemetryCollector().log_variable_access(name, 0) # Depth not easily tracked here without change
...
class Interpreter:
    ...
    def evaluate(self, expr: Expr):
        ...
        try:
            ...
            # reduces function call overhead (double dispatch)
            t = type(expr)
            if t is Literal:
                return expr.value
            elif t is Variable:
            ...
            # Log node execution
            self.telemetry.log_node_exec(type(expr).__name__, duration)
    ...
    def visit_Call(self, expr: Call):
        ...
        try:
            ...
            if hasattr(callee, 'declaration'):
                func_name = callee.declaration.name.lexeme
            else:
                func_name = str(callee)
            
            self.telemetry.log_function_call(func_name, duration, self.depth)
    ...
    def visit_Binary(self, expr: Binary):
        ...
        # Fast path for common numeric operations (Optimization Phase 2)
