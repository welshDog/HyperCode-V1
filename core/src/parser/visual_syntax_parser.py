...
class VisualSyntaxParser:
    ...
    def _build_semantic_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        Purpose:
        Constructs a dictionary of regex patterns to identify and parse semantic emoji markers in the code.
        Each marker is mapped to a specific `SemanticMarker` enum type.

        Returns:
            Dict[str, Dict[str, Any]]: A map where keys are emojis and values contain the compiled regex and marker type.

        Design Choice:
        Regex is used here for flexibility in parameter parsing, allowing markers to accept various argument formats.
        """
        patterns = {}
    ...
    def parse_content(self, content: str) -> List[ParsedFunction]:
        ...
        Purpose:
        Parses a raw string of HyperCode source code to extract function definitions and their associated semantic annotations.
        Handles the association of annotations (which may appear before the function) with the correct function object.

        Args:
            content (str): The full source code content.

        Returns:
            List[ParsedFunction]: A list of structured function objects with parsed metadata.
        """
        lines = content.split("\n")
        functions = []
    ...
    def extract_semantic_summary(
        ...
        Purpose:
        Aggregates metrics from the parsed functions, including counts of different semantic markers
        and an estimated cognitive load score. This data is used to provide feedback on code complexity.

        Args:
            functions (List[ParsedFunction]): The list of parsed functions to analyze.
    ...
    def validate_neurodiversity_compliance(
        ...
        Purpose:
        Checks the code against HyperCode's neurodiversity standards.
        Ensures functions have necessary accessibility and intent annotations, and warns if cognitive load (annotation density) is too high.

        Args:
            functions (List[ParsedFunction]): The list of parsed functions to validate.

        Returns:
            Dict[str, Any]: A compliance report with a score, list of issues, and recommendations.
        """
