...
def _create_token_patterns() -> List[Tuple[Pattern, Optional[TokenType]]]:
    """Create and return the token patterns in order of precedence."""
    return [
        ...
        # Numbers (floats before integers to prevent partial matches)
        (re.compile(r"\d+\.\d+"), TokenType.FLOAT),
        ...
        *[
            (re.compile(re.escape(char)), token_type)
            for char, token_type in [
                ("!", TokenType.BANG),
                ("=", TokenType.ASSIGN),
                ("<", TokenType.LESS),
                (">", TokenType.GREATER),
        ...
        # Identifiers and keywords (handled specially)
