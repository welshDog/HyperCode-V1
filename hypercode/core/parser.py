...
from .ast import (Assign, Binary, Block, Call, Expression, ExpressionStmt, Function, Grouping, If,
                  Literal, Print, Program, Return, Statement, Unary, Var, VarDecl, While)
...
class Parser:
    ...
    def synchronize(self) -> None:
        ...
        while not self.is_at_end():
            if (
                self.previous().type == TokenType.SEMICOLON
                or self.previous().type == TokenType.NEWLINE
            ):
                return
    ...
    def statement(self) -> Optional[Statement]:
        ...
        try:
            ...
            if not self.is_at_end():
                self.error(
                    self.peek(), f"Unexpected token in statement: {self.peek().type}"
                )
    ...
    def var_declaration(self) -> Statement:
        ...
        self.consume_newline_or_semicolon(
            "Expect newline or ';' after variable declaration."
        )
...
class Parser:
    ...
    def _expect(self, token_type: TokenType, message: str) -> Token:
        """Consume the next token if it matches the expected type, or raise an error."""
