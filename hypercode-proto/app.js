...
function setupEventListeners() {
  ...
  document.getElementById('theme-contrast').addEventListener('click', () => setTheme('high-contrast'));
  ...
  document.getElementById('toggle-panel').addEventListener('click', () => {
    ...
    document.getElementById('toggle-panel').textContent = panel.classList.contains('collapsed') ? '▲' : '▼';
  });
  ...
  document.getElementById('voice-submit').addEventListener('click', processVoiceCommand);
  ...
  state.tasks.forEach((task, index) => {
    ...
    taskEl.innerHTML = `
      ...
      <div class="task-actions">
        ${task.status !== 'done' ? `<button class="btn btn-sm btn-primary" data-task-index="${index}" data-action="done">✓ Complete</button>` : ''}
        ${task.status === 'todo' ? `<button class="btn btn-sm" data-task-index="${index}" data-action="progress">▶️ Start</button>` : ''}
...
function applyPreset(presetName) {
  ...
  if (preset.animations !== undefined) {
    ...
    document.documentElement.style.setProperty('--animation-speed', preset.animations / 100);
