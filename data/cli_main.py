...
@click.option('--sources', '-s', multiple=True, default=['arxiv', 'pubmed'],
...
def auto_research(query: str, sources: tuple, limit: int, continuous: bool):
    ...
    try:
        ...
        async def run():
            await manager.auto_research_mode(
                query=query,
                limit=limit
            )
...
def nodes(node_type: Optional[str], limit: int):
    ...
    try:
        with get_db_context() as session:
            ...
            nodes_list = query.order_by(
                KnowledgeNode.frequency.desc()
            ).limit(limit).all()
...
def relationships(rel_type: Optional[str], limit: int):
    ...
    from database_utils.db import get_db_context
    from database_utils.models import KnowledgeRelationship, KnowledgeNode
    ...
    try:
        with get_db_context() as session:
            ...
            relationships_list = query.order_by(
            ...
            for rel in relationships_list:
                source = session.query(KnowledgeNode).filter_by(id=rel.source_node_id).first()
                target = session.query(KnowledgeNode).filter_by(id=rel.target_node_id).first()
...
def graph(node_id: int):
    ...
    try:
        with get_db_context() as session:
            ...
            outgoing = session.query(KnowledgeRelationship).filter_by(
            ...
            incoming = session.query(KnowledgeRelationship).filter_by(
