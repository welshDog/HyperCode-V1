            ...
            for file in files:
                ...
                try:
                    if file.endswith(".py"):
                        ...
                        if entities:
                            print(f"  ✅ {relative_path}: " f"{len(entities)} entities")
...
"""
        ...
        for file_path in sorted(by_file.keys()):
            report += f"### {file_path}\n\n"
            for func in sorted(
                by_file[file_path], key=lambda f: f.get("lineno", 0)
...
def main() -> None:
    ...
    with open("HYPER_DATABASE.md", "w", encoding="utf-8") as f:
        f.write(md_report)
    print("   ✅ HYPER_DATABASE.md created")
    ...
    print("   ✅ HYPER_DATABASE.json created")
